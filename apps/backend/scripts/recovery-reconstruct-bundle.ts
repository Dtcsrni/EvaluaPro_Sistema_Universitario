...
import { reconstruirDesdeBundle } from '../src/modulos/modulo_recuperacion_examenes/servicioRecuperacionExamenes.js';
...
function parseArgs(argv: string[]): Args {
  ...
  if (!args.bundleHash && !args.loteId) {
    throw new Error('Debes indicar --bundle-hash o --lote-id');
  }
  return args;
}
...
async function main() {
  ...
  try {
    const result = await reconstruirDesdeBundle({
