...
import { conectarBaseDatos } from '../src/infraestructura/baseDatos/mongoose.js';
import { reconstruirDesdeManifest } from '../src/modulos/modulo_recuperacion_examenes/servicioRecuperacionExamenes.js';
...
function parseArgs(argv: string[]): Args {
  ...
  if (!args.manifestHash && !args.examId && !args.folio) {
    throw new Error('Debes indicar --manifest-hash o --exam-id o --folio');
  }
  return args;
}
...
async function main() {
  ...
  try {
    const result = await reconstruirDesdeManifest({
