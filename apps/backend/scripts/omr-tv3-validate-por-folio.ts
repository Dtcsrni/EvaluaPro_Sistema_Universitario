...
function parseArgs(argv: string[]): Args {
  const args: Args = {
    ...
    report: '../../reports/qa/latest/omr/tv3-por-folio-validation.json',
    failureReport: '../../reports/qa/latest/omr/tv3-por-folio-failures.json'
  };
...
export async function runTv3PorFolioValidation(args: {
  datasetRoot: string;
  reportPath: string;
  ...
  for (const capture of manifest.capturas) {
    ...
    const result = await analizarOmr(
      ...
      mapPage,
      [capture.expectedQr, capture.folio, `EXAMEN:${capture.folio}:P${capture.numeroPagina}:TV3`],
      10,
      {
        folio: capture.folio,
  ...
  const runId = `omr-tv3-por-folio-${Date.now()}`;
  const report = {
    runId,
...
async function main() {
  ...
  const { report } = await runTv3PorFolioValidation({
