...
- **Elevación Radical Bento de todas las Pantallas del Shell Docente (`SPEC-BENTO-ELEVATION-ACADEMICA`)**:
  ...
  - **Calificaciones y Escrutinio OMR (`SeccionCalificaciones.tsx`, `GuiaCalificacionesVisual.tsx`)**: Bento Hero Header con orbe violeta/púrpura, status pill de mesa de calificación, Mini-KPIs (`En flujo`, `Procesadas`, `Pendientes`, `Calificados`, `Solicitudes`), guía Bento Step Cards (*Lectura Óptica OMR*, *Resolución Rápida*, *Cálculo y Publicación*) y atajos de teclado para escrutinio milimétrico.
  - **Evaluaciones y Políticas (`SeccionEvaluaciones.tsx`, `GuiaEvaluacionesVisual.tsx`)**: Bento Hero Header con orbe cobalto/azul, status pill de analítica académica, Mini-KPIs (`Políticas`, `Alumnos`, `Sección`), guía Bento Step Cards (*Consolidación de Notas*, *Reactivos Críticos*, *Sábanas y Reportes*) y tabulación analítica limpia.
  - **Sincronización y Backups (`SeccionSincronizacion.tsx`, `GuiaSincronizacionVisual.tsx`)**: Bento Hero Header con orbe esmeralda/verde menta, status pill de sincronización híbrida & offline, Mini-KPIs (`Materias`, `Alumnos`, `Plantillas`, `Exitosas`), guía Bento Step Cards (*Emparejamiento de Equipos*, *Paquetes de Respaldo*, *Fusión No Destructiva*) y panel de trazabilidad de operaciones.
  - **Cuenta y Perfil (`SeccionCuenta.tsx`, `GuiaCuentaVisual.tsx`)**: Bento Hero Header con orbe zafiro/plateado, status pill de identidad institucional, Mini-KPIs de credenciales e integraciones (`Rol`, `Google`, `Contraseña`, `PDF Header`, `SMTP`), guía Bento Step Cards (*Perfil e Institución*, *Licencia Institucional*, *Seguridad & Criptografía*) y depuración de 3 acordeones redundantes.
  - **Banco de Preguntas (`SeccionBanco.tsx`, `BancoFormularioPregunta.tsx`, `BancoGestionTemas.tsx`, `BancoListadoPreguntas.tsx`, `GuiaBancoVisual.tsx`)**: Reestructuración completa con Hero Header Bento, orbe neón animado, Mini-KPIs vectoriales con hover lift (`Preguntas`, `Temas`, `Sin tema`, `Tema actual`, `Páginas est.`), selectores animados, contador dinámico de caracteres, zona de upload con previsualización, selector de opciones A–E con avatares cromáticos y radio pills con halo luminoso, y guía Bento Step Cards de 3 pasos con persistencia.
  - **Temarios (`SeccionTemarios.tsx`, `GuiaTemariosVisual.tsx`)**: Hero Header Bento, Mini-KPIs (`Temarios`, `Temas`, `% Avance`, `Cubiertos`), estado vacío interactivo con selector de asignaturas, árbol curricular jerárquico multinivel con semáforo interactivo y drag & drop para programas en PDF.
  - **Asistencias (`SeccionAsistencias.tsx`, `GuiaAsistenciasVisual.tsx`)**: Hero Header Bento con orbe esmeralda pulsante, Mini-KPIs ejecutivos (`Sesiones`, `Alumnos`, `Asist. Promedio`, `Retardos`, `Faltas`), barra de progreso con gradiente esmeralda/ámbar, selector de fecha rápida, barra de búsqueda en tiempo real con resaltado, avatares circulares de alumnos y botones rápidos "Marcar todos presentes / ausentes".
  - **Materias y Alumnos (`SeccionPeriodos.tsx`, `SeccionAlumnos.tsx`, `GuiaMateriaVisual.tsx`, `GuiaAlumnoVisual.tsx`)**: Eliminación de acordeones obsoletos, banners duplicados y métricas redundantes (correo institucional unificado), sustituidos por Bento Step Cards interactivas con persistencia y orbes de gradiente.
- Propagación Glassmorphism Integral, Ergonomía Visual Multi-Pantalla y Suite 100% Verde (`SPEC-REDISENIO-INTEGRAL-GUI`):
  - **Propagación Glassmorphic a Todo el Sistema**: Migración exhaustiva de backgrounds y bordes hardcodeados hacia el sistema jerárquico de variables `--glass-surface-0/1/2/3`, `--glass-border` y `--glass-shadow` en todas las pantallas y componentes (Shell Docente, Portal Alumno, Autenticación, Materias, Banco, Plantillas, Calificaciones, Asistencias, Version Center y Empty States).
  - **Coherencia y Contraste en Tema Claro**: Añadidos más de 105 selectores de tema claro (`:root:not([data-theme="dark"])`) con superficies translúcidas blancas de alta legibilidad, saturación equilibrada y contraste verificado.
  - **Formulario Panorámico de Materias y Bento Workspace**: Rediseño estructurado en 2 filas limpias con aprovechamiento del 100% del ancho de pantalla, eliminando solapamientos y desbordamientos en cualquier resolución.
  - **Dock de Navegación y Paletas Cromáticas por Sección**: Integración de 11 paletas cromáticas personalizadas (`data-icono-tab`) con íconos SVG únicos, animaciones pop-in y resplandor al interactuar.
  - **Higiene de Código y Cero Inline Styles**: Desacoplada la barra de robustez de contraseñas hacia selectores CSS semánticos con `data-nivel`, cumpliendo la auditoría `gui.responsive.audit.test.ts` con 0 violaciones.
  - **Suite de Pruebas Unitarias de Frontend 100% en Verde**: Corregidos contratos de renderizado en `VersionInfoPage`, `VersionInfo.helpers`, `SeccionAutenticacion` y `SeccionPeriodos`, alcanzando 55 suites y 201 pruebas unitarias aprobadas sin regresiones.
- Rediseño Integral de GUI, Wireframes Maestros y Sistema Glassmorphism (`SPEC-REDISENIO-INTEGRAL-GUI`): Implementación de la especificación SDD `docs/specs/redisenio_integral_gui_nordic_slate.spec.md` con elevación visual, layout de alto contraste y ergonomía operativa en todo el frontend:
  - **Arquitectura de Layout Master Sidebar & Multi-Pane**: Reestructuración del layout docente con navegación en dock de cristal esmerilado (*Glass Dock*), cabecera ejecutiva con búsqueda, selector de ciclo y perfil, y área de trabajo dividida en columnas para evitar el scroll vertical innecesario.
  - **Sistema Cromático Prismatic Sapphire & Aurora Starlight**: Integración de la paleta oscura de lujo con lienzo en azul marino abisal (`#060B14`), superficies en zafiro translúcido (`rgba(15, 29, 54, 0.78)`), pestañas y botones en gradiente **Cobalto ➔ Cian Neón (`#2563EB` ➔ `#00D2FF`)**, y jerarquía de métricas KPI en **Cian Neón (`#00D2FF`)**, **Violeta Eléctrico (`#A78BFA`)** y **Ámbar Solar (`#FBBF24`)** con sombras difusas para lectura instantánea.
  - **Superficies Glassmorphic y Refracción Especular**: Implementación de `backdrop-filter: blur(20px)`, bordes de cristal fino (`rgba(56, 189, 248, 0.20)`) y reflejos interiores especulares (`inset 0 1px 1px 0 rgba(255, 255, 255, 0.12)`) en todas las tarjetas y paneles del sistema.
  - **Catálogo Maestro de Wireframes Visuales**: Generación y aprobación del catálogo arquitectónico con planos visuales de alta resolución para el Hub de Materias (con *slide-over drawer* lateral), Banco de Reactivos (con renderizado KaTeX en tiempo real y previsualización dual PDF/OMR) y Hub de Calificación y Escrutinio OMR Instantáneo (con visor de burbujas y tabla de resolución interactiva).
  - **Validación Automatizada 100% en Verde**: Verificación de contratos de diseño CSS (`2/2`), 200 pruebas unitarias de frontend (`55/55` archivos), 13 pruebas Playwright responsive en 4 resoluciones (Desktop, Tablet, Tablet-SM y Mobile) y 17 verificaciones de contrato de pipeline CI/CD sin errores.
- Refactorización y Modernización Integral de Arquitectura y UX/UI (`SPEC-ARCH-UX-MODERNIZATION`): Implementación de la especificación SDD `docs/specs/arquitectura_ux_refactor_integral.spec.md` con 10 requerimientos cumplidos al 100%.
  - **Desacoplamiento Frontend**: Creación de hooks especializados `useOmrWorkflowState` (aislamiento completo de estados, cálculos autoritativos y acciones OMR), `useRecursosAcademicosDocente` (gestión modular de alumnos, periodos/materias activas y archivadas, plantillas y banco de preguntas) y `usePlantillasPreviewState` (gestión reactiva de previsualizaciones JSON/PDF y páginas estimadas por tema), reduciendo drásticamente la complejidad ciclomática y acoplamiento de `AppDocente.tsx` (reducción de 1350 a 914 líneas).
  - **Ampliación de Cobertura Unitarias**: Implementación y expansión de suites de pruebas unitarias para `useOmrWorkflowState`, `useRecursosAcademicosDocente`, `usePlantillasPreviewState`, `SeccionTemarios`, `SeccionEvaluaciones`, `PlantillasFormulario/Listado/Generados/OmrWorkflow`, `BancoListado/Formulario/GestionTemas`, `SeccionAlumnos`, `SeccionCuenta`, `SeccionPublicar`, `SeccionEntrega`, `SeccionRehidratacionLotes`, `SeccionCalificar`, `SeccionSincronizacionEquipos`, `SeccionPaqueteSincronizacion` y `QrAccesoMovil` (alcanzando 200 tests de frontend pasando al 100% de diff coverage).
  - **Higiene de Código y Cero Inline Styles**: Eliminados todos los estilos en línea (`style={{...}}`) y colores hardcodeados de `SeccionAsistencias.tsx`, `SeccionTemarios.tsx` y `AppDocente.tsx`, migrándolos a clases CSS semánticas e institucionales en `components.css`. Auditoría `gui.responsive.audit.test.ts` pasando con 0 violaciones.
  - **Ergonomía UX y Atajos de Teclado OMR**: Implementados atajos rápidos de teclado (`A`, `B`, `C`, `D`, `E`, `0`/`Delete`/`-`) en la mesa de revisión OMR (`SeccionEscaneo.tsx`) para corrección de reactivos a velocidad de digitación.
  - **Resiliencia de Entorno en Laptops**: Ajustados los umbrales de resolución mínima en el Bootstrapper WPF (`MainWindow.xaml.cs`) a `1024x576 DIPs`, habilitando soporte nativo en pantallas de 1366x768 / 1280x720 con escalado DPI de 125%–150%.
  - **Optimización y Replantante de Dependencias**: Eliminado `sharp` duplicado en `dependencies` de la raíz del monorepo, actualizados paquetes `sharp` a `0.35.3`, `express-rate-limit` a `8.6.2`, `@types/react` a `19.2.18`, `@types/react-dom` a `19.2.4` y `@testing-library/user-event` a `14.6.5`.
  - **Cobertura TDD Integral**: Creada suite `seccionTemarios.test.tsx` y extendidas `seccionAsistencias.test.tsx` y `escaneo.refactor.test.tsx`, logrando 100% diff coverage y 40 archivos de prueba en frontend con 155 tests pasando.
